...
const HEADERS = { 'channel': 'minApp', 'device-type': 'web', 'platformtype': 'web', 'token': TOKEN, 'Content-Type': 'application/json' };

async function testApi() {
  try {
    const res = await fetch(BASE + '/etoken/reward/balance', { method: 'POST', headers: HEADERS, body: '{}' });
    const data = await res.json();
    console.log('/etoken/reward/balance =>', JSON.stringify(data, null, 2));
  } catch(e) {}
}
